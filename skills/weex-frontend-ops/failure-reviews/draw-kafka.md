...
## 2026-05-29 二次权重专项脚本误点抽奖控件导致跳转

- 日期：2026-05-29
- 页面/流程：活动 `9107` 前端二次权重专项，目标是连续单抽 6 次并断言第 6 次命中奖品 ID `5`。
- 失败表现：旧专项脚本定位 `抽奖 × 1` 不稳定；一次兜底 DOM 点击过宽，可能误点页面导航区域并进入 `https://stg-www.weex.tech/zh-CN/autoearn`；同时兜底点击使用 `scrollIntoView`，导致每次抽奖后页面上下滚动。
- 失败原因：抽奖控件不是始终能被简单 `button` locator 命中，脚本不应扩大到任意包含 `抽奖` 文案的父级 DOM，更不能点击大容器或导航节点。
- 解决方式：二次权重专项脚本改为只允许点击当前视口内、可见、可点击且文本精确等于 `抽奖×1` 的最小控件；不再滚动页面、不再用宽泛父节点兜底。关闭中奖弹窗前必须先确认 `恭喜你` 弹窗可见，避免误点其他确认按钮。
- 验证结果：2026-05-29 使用新活动 `9694` / `wt43083858`、账号 `8186595891@weex.com` / UID `8186595891` 复验通过；MQ 回调后抽奖次数为 `10`，连续单抽 6 次均捕获 `恭喜你` 弹窗，次数从 `10` 降到 `4`，第 6 次弹窗文案为 `100 USDT 合约赠金`。
- 关联流程或脚本：`scripts/frontend-draw-weight-special-verify.mjs`。
- 后续处理状态：已吸收到固定流程；后续二次权重前端验证必须等待中奖弹窗出现，读取弹窗奖品文案，再点击弹窗右上角 `X` 关闭后继续下一次抽奖，不关闭页面、不滚动、不跳转。
