...
| Frontend authenticated asset transfer depends on loginTool token session | `skills/weex-frontend-ops/.env.local` account + `/Users/gabriel/Downloads/weexpr/loginTool` | `POST https://stg-gateway2.weex.tech/v1/assets/transfer` | Transfer API requires a frontend login access token sent as `U-Token`; token stays in memory and output is redacted | candidate | 2026-05-08 | Default STG example moves amount `1000` from account type `10` to account type `8` with coin id `2`; real execution requires `--confirm-transfer`. |
