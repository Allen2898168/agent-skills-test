...
| STG draw task callback verification depends on Kafka callback and frontend task APIs | Internal Kafka topic `flink.exchange.spot_capital_order_info` | Frontend draw task APIs `/v1/activity/general/taskCompletions` and `/v1/activity/general/raffle/frequency` | Kafka produce can trigger backend task completion, but frontend success must still be verified with the logged-in account task APIs and visible task progress | candidate | 2026-05-13 | For Kafka UI sends, fill only Value unless the user explicitly asks for key or headers. |
