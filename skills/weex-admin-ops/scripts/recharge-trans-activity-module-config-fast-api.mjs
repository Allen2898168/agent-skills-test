...
function usage() {
  ...
  node skills/weex-admin-ops/scripts/recharge-trans-activity-module-config-fast-api.mjs --action snapshot --activity-id <id>
...
Spec:
  {
    ...
    "modules": {
      ...
      "tasks": { "taskConfig": [ { "id": 123, "order": 1 } ] },
      "rules": { "expanded": 0, "activityConfigI18n": [ ... ] },
...
function loadModuleNameMap() {
  return loadMarkdownMap("skills/weex-admin-ops/references/mappings/recharge-trans-activity-modules.md", "模块 key", "前端中文名");
}

function loadFieldNameMap() {
  return loadMarkdownMap("skills/weex-admin-ops/references/mappings/recharge-trans-activity-fields.md", "字段 key", "前端中文名");
}

async function findByAlias(api, alias) {
  const list = await api.get(`/prod-api/activity/config/list?pageNum=1&pageSize=10&type=RECHARGE_TRANS_TASK&showUrl=${encodeURIComponent(alias)}`);
...
function applyModulesToDetail(current, spec) {
  ...
  if (modules.rules && typeof modules.rules === "object") {
    assignIfPresent(patched, modules.rules, "expanded");
    if (Array.isArray(modules.rules.activityConfigI18n)) patched.activityConfigI18n = modules.rules.activityConfigI18n;
  }
  ...
  return {
    ...
    current: {
      base: {
        ...
        webBannerUrl: item.webBannerUrl ?? null,
        appBannerUrl: item.appBannerUrl ?? null,
        webShareUrl: item.webShareUrl ?? null,
        appShareUrl: item.appShareUrl ?? null,
        webMp4Files: item.webMp4Files ?? null,
        appMp4Files: item.appMp4Files ?? null,
        ogImageUrl: item.ogImageUrl ?? null,
        shareContent: item.shareContent ?? null,
        agentShareContent: item.agentShareContent ?? null,
      ...
      rules: { expanded: item.expanded ?? null, activityConfigI18n: Array.isArray(item.activityConfigI18n) ? item.activityConfigI18n : [] },
...
async function run() {
  ...
  if (args.wizard || !args.action) {
    ...
    printJson({
      ...
      dryRun: true,
      wizard: {
        domain: "活动列表 / 充值交易活动(RECHARGE_TRANS_TASK) 模块级配置（headless_api）",
        moduleNameMap: moduleNameMap.map,
        moduleNameMapSource: moduleNameMap.source,
        fieldNameMap: fieldNameMap.map,
        fieldNameMapSource: fieldNameMap.source,
        ...
        notes: [
          ...
          "rules 模块中的多语言规则标题/内容存放在 activityConfigI18n[*].ruleTitle 与 activityConfigI18n[*].intro；建议直接在 spec.modules.rules.activityConfigI18n 里整体提供。",
        ],
      },
    });
    return 0;
