...
function usage() {
  ...
  node skills/weex-admin-ops/scripts/agent-activity-module-config-fast-api.mjs --action snapshot --activity-id <id>
...
function loadModuleNameMap() {
  return loadMarkdownMap("skills/weex-admin-ops/references/mappings/agent-activity-modules.md", "模块 key", "前端中文名");
}

function loadFieldNameMap() {
  return loadMarkdownMap("skills/weex-admin-ops/references/mappings/agent-activity-fields.md", "字段 key", "前端中文名");
}

async function findByAlias(api, alias) {
  const list = await api.get(`/prod-api/activity/config/list?pageNum=1&pageSize=10&type=AGENT&showUrl=${encodeURIComponent(alias)}`);
...
function buildWizardTemplate() {
  return {
    ...
    modules: {
      base: { title: "", subTitle: "", showUrl: "", startTime: "", endTime: "", channelCategory: "", applyConfigId: 0, taskConfig: [] },
      calendar: { syncCalendarFlag: 0, syncCalendarDto: {} },
    },
    rawTopLevel: {},
  };
  ...
  return {
    ...
    current: {
      base: {
        ...
        activityLabelList: item.activityLabelList ?? null,
        taskConfig: Array.isArray(item.taskConfig) ? item.taskConfig : [],
        showEndCountdown: item.showEndCountdown ?? null,
        showActivityCalendar: item.showActivityCalendar ?? null,
        activityConfigI18n: Array.isArray(item.activityConfigI18n) ? item.activityConfigI18n : [],
      },
...
async function run() {
  ...
  if (args.wizard || !args.action) {
    ...
    printJson({
      ...
      dryRun: true,
      wizard: {
        domain: "活动列表 / 人人代理活动(AGENT) 模块级配置（headless_api）",
        moduleNameMap: moduleNameMap.map,
        moduleNameMapSource: moduleNameMap.source,
        fieldNameMap: fieldNameMap.map,
        fieldNameMapSource: fieldNameMap.source,
        ...
        notes: [
          ...
          "任务配置在顶层 taskConfig；该字段为数组，建议整体提供。",
        ],
      },
    });
    return 0;
