...
function usage() {
  return `Usage:
  # dry-run
  node skills/weex-admin-ops/scripts/create-competition-trading-volume-task-from-scratch-fast-api.mjs --required-volume 1 --dry-run

  # create
  node skills/weex-admin-ops/scripts/create-competition-trading-volume-task-from-scratch-fast-api.mjs --required-volume 1 --confirm-create

Options:
  --required-volume <n>   required; 交易量阈值（USDT）
  --name-prefix <text>    default 交易大赛_交易量
  --tag-prefix <text>     default tctr_s
...
function parseArgs() {
  ...
  args.tagPrefix = args.tagPrefix ? String(args.tagPrefix) : "tctr_s";
...
function buildTaskPayload(args) {
  ...
  return {
    ...
    remark: (args.remark || `自动化-交易大赛交易量任务(${args.requiredVolume}U)`).slice(0, 120),
    conditions: {
      allowAgentList: [],
      allowUserList: [],
      allowAreaList: [],
...
async function run() {
  ...
  const plan = {
    mode: "headless_api",
    activityType: "TRADING_COMPETITION(2)",
  ...
  try {
    ...
    printJson({
      ...
      created: { id: String(row.id), name: payload.name, tag: payload.label, requiredVolume: args.requiredVolume },
