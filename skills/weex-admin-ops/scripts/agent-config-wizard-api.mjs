...
function usage() {
  ...
  node skills/weex-admin-ops/scripts/agent-config-wizard-api.mjs --wizard

  # Step 2: execute after confirmation (spec required)
  node skills/weex-admin-ops/scripts/agent-config-wizard-api.mjs --spec-file ./tmp/agent-spec.json --confirm
...
function loadAgentModuleNameMap() {
  const refPath = path.join(repoRoot, "skills/weex-admin-ops/references/mappings/agent-activity-modules.md");
  if (!fs.existsSync(refPath)) return { source: "missing", map: {} };
  const md = fs.readFileSync(refPath, "utf8");
  return { source: "references/mappings/agent-activity-modules.md", map: parseMarkdownTableToMap(md, "模块 key", "前端中文名") };
}

function loadAgentFieldNameMap() {
  const refPath = path.join(repoRoot, "skills/weex-admin-ops/references/mappings/agent-activity-fields.md");
  if (!fs.existsSync(refPath)) return { source: "missing", map: {} };
  const md = fs.readFileSync(refPath, "utf8");
  return { source: "references/mappings/agent-activity-fields.md", map: parseMarkdownTableToMap(md, "字段 key", "前端中文名") };
}

function buildWizardMenu() {
  const moduleNameMap = loadAgentModuleNameMap();
  const fieldNameMap = loadAgentFieldNameMap();
  ...
  return {
    domain: "活动列表 / 人人代理活动(AGENT) API 向导（进行中）",
    ...
    presets: [
      ...
      { preset: "online", risk: "高", description: "上线指定人人代理活动（/activity/agent/online）" },
      { preset: "offline", risk: "高", description: "下线指定人人代理活动（/activity/agent/offline）" },
      { preset: "delete", risk: "高", description: "删除指定人人代理活动（/activity/agent/delete）" },
    ],
    ...
    notes: [
      ...
      "模块级自由配置脚本：agent-activity-module-config-fast-api.mjs（snapshot -> spec -> update）。",
    ],
  };
}
...
function planPreset(args) {
  ...
  const base = "skills/weex-admin-ops/scripts/agent-activity-fast-api.mjs";

  if (preset === "full_create_verify_delete") {
    const full = "skills/weex-admin-ops/scripts/create-agent-full-config-explicit-deps-fast-api.mjs";
    return {
      steps: [
        {
          script: full,
