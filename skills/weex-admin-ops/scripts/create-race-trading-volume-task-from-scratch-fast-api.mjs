...
function usage() {
  return `Usage:
  # dry-run
  node skills/weex-admin-ops/scripts/create-race-trading-volume-task-from-scratch-fast-api.mjs --required-volume 1 --award-prize-id 123 --dry-run

  # create
  node skills/weex-admin-ops/scripts/create-race-trading-volume-task-from-scratch-fast-api.mjs --required-volume 1 --award-prize-id 123 --confirm-create

Options:
  --required-volume <n>   required; 交易量阈值（USDT）
  --award-prize-id <id>   required; 竞速赛阶段奖品ID（建议 GIFT_CASH）
  --award-amount <n>      default 1
  --name-prefix <text>    default 竞速赛_交易量
  --tag-prefix <text>     default rctr_s
...
function parseArgs() {
  ...
  args.namePrefix = args.namePrefix ? String(args.namePrefix) : "竞速赛_交易量";
  args.tagPrefix = args.tagPrefix ? String(args.tagPrefix) : "rctr_s";
...
function buildTaskAward({ awardPrizeId, awardAmount }) {
  return {
    awardMode: "SINGLE",
...
function buildTaskPayload(args) {
  ...
  return {
    ...
    resetDay: null,
    resetTime: null,
    taskAward: buildTaskAward({ awardPrizeId: args.awardPrizeId, awardAmount: args.awardAmount }),
    ...
    remark: (args.remark || `自动化-竞速赛交易量任务(${args.requiredVolume}U)`).slice(0, 120),
    conditions: {
      allowAgentList: [],
      allowUserList: [],
      allowAreaList: [],
...
async function run() {
  ...
  const plan = {
    mode: "headless_api",
    activityType: "RACE_COMPETITION(20)",
