...
function usage() {
  return `Usage:
  # dry-run
  node skills/weex-admin-ops/scripts/create-roulette-spot-volume-task-fast-api.mjs --required-volume 2 --dry-run

  # create
  node skills/weex-admin-ops/scripts/create-roulette-spot-volume-task-fast-api.mjs --required-volume 2 --confirm-create

Options:
  --required-volume <n>    required; 现货交易量阈值（USDT）
  --name-prefix <text>     default 转盘抽奖_spt
  --tag-prefix <text>      default rcspt
...
function parseArgs() {
  ...
  args.tagPrefix = args.tagPrefix ? String(args.tagPrefix) : "rcspt";
  args.remark = args.remark ? String(args.remark) : "";
  return args;
}

async function findTemplate(api) {
  const list = await api.get(`/prod-api/activity/task/list?name=${encodeURIComponent("转盘抽奖_spot_")}&pageNum=1&pageSize=1`);
  const row = firstRow(list);
  const id = row?.id;
  if (!id) throw new Error("Task template not found: 转盘抽奖_spot_");
...
function mutateTaskPayload(payload, args) {
  ...
  payload.name = name;
  payload.content = `现货交易量≥${args.requiredVolume}U`.slice(0, 120);
  payload.label = tag;
  payload.remark = (args.remark || `自动化-现货交易量任务(${args.requiredVolume}U)`).slice(0, 120);
...
function patchRequirement(payload, requiredVolume) {
  ...
  if (!("requiredVolume" in payload.requirement[0])) throw new Error("Task template requirement[0].requiredVolume is missing");
