...
function usage() {
  return `Usage:
  # dry-run
  node skills/weex-admin-ops/scripts/create-roulette-contract-volume-task-from-scratch-fast-api.mjs --required-volume 1 --lottery-count-prize-id 123 --dry-run

  # create
  node skills/weex-admin-ops/scripts/create-roulette-contract-volume-task-from-scratch-fast-api.mjs --required-volume 1 --lottery-count-prize-id 123 --confirm-create
...
Options:
  ...
  --tag-prefix <text>            default rcctr_s
...
function parseArgs() {
  ...
  args.tagPrefix = args.tagPrefix ? String(args.tagPrefix) : "rcctr_s";
...
function buildTaskPayload(args) {
  ...
  return {
    ...
    taskUrlWeb: null,
    taskUrlApp: null,
    remark: (args.remark || `自动化-合约交易量任务(${args.requiredVolume}U)`).slice(0, 120),
    conditions: {
      allowAgentList: [],
      allowUserList: [],
      allowAreaList: [],
