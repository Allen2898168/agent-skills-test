...
test("frontend main regression keeps frontend session manual until draw-page auth is stable", () => {
  ...
  assert.equal(byKey.get("FRONTEND_SESSION")?.autoHandled, false);
  assert.equal(byKey.get("NORMAL_ACTIVITY_ONLINE")?.autoHandled, true);
  assert.equal(byKey.get("NORMAL_ACTIVITY_SIGNED_UP")?.autoHandled, false);
  assert.equal(byKey.get("NORMAL_ACTIVITY_DRAW_GE1")?.autoHandled, false);
  assert.equal(byKey.get("REWARD_RECORD_DELAY_READY")?.autoHandled, false);
});
