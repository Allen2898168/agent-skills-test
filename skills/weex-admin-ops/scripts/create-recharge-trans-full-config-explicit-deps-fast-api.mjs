...
function usage() {
  return `Usage:
  # Plan only (no writes)
  node skills/weex-admin-ops/scripts/create-recharge-trans-full-config-explicit-deps-fast-api.mjs --dry-run

  # Create full-config RECHARGE_TRANS_TASK with explicitly created deps (writes)
  node skills/weex-admin-ops/scripts/create-recharge-trans-full-config-explicit-deps-fast-api.mjs --confirm-create

  # Create + cleanup (recommended for validation runs)
  node skills/weex-admin-ops/scripts/create-recharge-trans-full-config-explicit-deps-fast-api.mjs --confirm-create --cleanup --confirm-cleanup
...
Options:
  ...
  --cleanup
  --confirm-cleanup
  --template-alias <alias>    optional; if omitted, auto-pick first RECHARGE_TRANS_TASK activity as template baseline
  --title-prefix <text>       default 充值交易全配
  --alias-prefix <text>       default rt
...
function parseArgs() {
  ...
  args.titlePrefix = args.titlePrefix ? String(args.titlePrefix) : "充值交易全配";
  args.aliasPrefix = args.aliasPrefix ? String(args.aliasPrefix) : "rt";
...
async function resolveTemplate(api, templateAlias) {
  if (templateAlias) {
    const list = await api.get(`/prod-api/activity/config/list?pageNum=1&pageSize=1&type=RECHARGE_TRANS_TASK&showUrl=${encodeURIComponent(templateAlias)}`);
  ...
  const list = await api.get("/prod-api/activity/config/list?pageNum=1&pageSize=1&type=RECHARGE_TRANS_TASK");
  const row = firstRow(list);
  const id = row?.activityId || row?.id;
  if (!id) throw new Error("未找到可用于 clone 的 RECHARGE_TRANS_TASK 活动模板（活动列表为空）；请先在后管创建至少 1 个充值交易活动。");
  const detail = await api.get(`/prod-api/activity/config/${encodeURIComponent(id)}`);
  if (detail.body?.code !== 200 || !detail.body?.data) throw new Error(`Template detail failed: ${id}`);
  return { id: String(id), alias: String(detail.body.data?.showUrl || row?.showUrl || ""), detail: detail.body.data };
}

function patchActivityI18n(payload, { title, subTitle, ruleTitle, intro }) {
  if (Array.isArray(payload.activityConfigI18n) && payload.activityConfigI18n.length) {
    payload.activityConfigI18n = payload.activityConfigI18n.map(item => ({ ...item, title, subTitle, ruleTitle, intro }));
    return;
  }
  payload.activityConfigI18n = [{ lang: "zh_CN", title, subTitle, ruleTitle, intro }];
}

function buildActivityPayloadFromTemplate({ templateDetail, args, created }) {
  const ts = buildSuffix();
  const alias = String(`${args.aliasPrefix || "rt"}${Date.now().toString().slice(-8)}`).slice(0, 32);
  const title = String(`${args.titlePrefix || "充值交易活动"}${ts.slice(-6)}`).slice(0, 120);
  const subTitle = String(`自动化副标题_${ts.slice(-6)}`).slice(0, 120);
  const ruleTitle = String(`活动规则_${ts.slice(-6)}`).slice(0, 120);
  const intro = `自动化规则内容_${ts.slice(-6)}`;
  ...
  payload.expanded = payload.expanded ?? 0;
  payload.showActivityCalendar = payload.showActivityCalendar ?? 1;
  patchActivityI18n(payload, { title, subTitle, ruleTitle, intro });
  return { alias, title, subTitle, window, payload };
}

async function deleteActivity(api, activityId, config) {
  const del = await api.post("/prod-api/activity/rechargeTrans/delete", { activityId: Number(activityId), totp: String(config.googleCode || "") });
...
async function run() {
  ...
  const plan = {
    ...
    dependencyCreates: {
      ...
      task: `create-recharge-trans-trading-volume-task-fast-api.mjs (requiredVolume=${args.requiredVolume}U)`,
    },
    activity: {
  ...
  if (!args.confirmCreate) throw new Error("需要用户确认：请加 --confirm-create 后才允许创建充值交易活动全配置与依赖模块。");
  ...
  try {
    ...
    const task = await runChildJson([
      "skills/weex-admin-ops/scripts/create-recharge-trans-trading-volume-task-fast-api.mjs",
      "--required-volume",
      String(args.requiredVolume),
      "--confirm-create",
    ]);
    if (!task.ok) throw new Error(`create-recharge-trans-trading-volume-task-fast-api.mjs failed: ${task.json?.error || "unknown"}`);
    created.taskId = String(task.json?.created?.id || "");
    created.taskDetail = task.json?.createdTaskDetail || null;
    if (!created.taskId || !created.taskDetail) throw new Error("No task id/detail returned from create-recharge-trans-trading-volume-task-fast-api.mjs");
    ...
    if (create.body?.code !== 200) throw new Error(`Create RECHARGE_TRANS_TASK activity failed: ${JSON.stringify(create.body)}`);
    const row = firstRow(await api.get(`/prod-api/activity/config/list?pageNum=1&pageSize=1&type=RECHARGE_TRANS_TASK&showUrl=${encodeURIComponent(built.alias)}`));
    ...
    const draftChecks = await runChildJson([
      "skills/weex-admin-ops/scripts/recharge-trans-activity-fast-api.mjs",
      "--action",
      "draft-checks",
      "--activity-id",
      String(activityId),
    ]);
    if (!draftChecks.ok) throw new Error(`recharge-trans-activity-fast-api.mjs draft-checks failed: ${draftChecks.json?.error || "unknown"}`);
    ...
    if (verifyLevel === "full") {
      ...
      if (!online.ok) throw new Error(`recharge-trans-activity-fast-api.mjs online failed: ${online.json?.error || "unknown"}`);
      ...
      if (!offline.ok) throw new Error(`recharge-trans-activity-fast-api.mjs offline failed: ${offline.json?.error || "unknown"}`);
    ...
    const evidence = {
      ok: true,
      mode: "headless_api",
      finalUrl: `${config.baseUrl}/activities/depositTrade`,
      created: {
        activityId,
        showUrl: built.alias,
        title: built.title,
