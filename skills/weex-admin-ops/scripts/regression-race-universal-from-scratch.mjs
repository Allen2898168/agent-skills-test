...
function usage() {
  return `Usage:
  # dry-run (no writes)
  node skills/weex-admin-ops/scripts/regression-race-universal-from-scratch.mjs --dry-run

  # run (writes)
  node skills/weex-admin-ops/scripts/regression-race-universal-from-scratch.mjs --confirm-run

Options:
  --title-prefix <text>       default 竞速赛通用回归
  --alias-prefix <text>       default rcr
...
function parseArgs() {
  ...
  args.titlePrefix = args.titlePrefix ? String(args.titlePrefix) : "竞速赛通用回归";
  args.aliasPrefix = args.aliasPrefix ? String(args.aliasPrefix) : "rcr";
...
async function uploadImgReplace({ baseUrl, authorization, imagePath }) {
  ...
  if (Number(body?.code) !== 200) {
    throw new Error(`uploadImgReplace not accepted: ${JSON.stringify({ code: body?.code, msg: body?.msg || body?.message })}`);
  }
...
function isAuthFailureResponse(res) {
  const code = res?.body?.code ?? null;
  const msg = String(res?.body?.msg || res?.body?.message || "");
  return Number(code) === 401 && /认证失败|unauthorized/i.test(msg);
}

async function createActivityConfigWithRetry(api, payload, { maxRetries = 3 } = {}) {
  const backoffsMs = [1200, 2500, 5000, 9000, 15000, 20000, 25000];
  let last = null;
  for (let attempt = 0; attempt <= Math.max(0, Number(maxRetries) || 0); attempt += 1) {
    try {
      ...
      last = { status: 0, body: { code: 500, msg: `exception: ${error?.message || String(error)}` } };
    }
...
function buildRequirements() {
  return [{ type: "BY_PRODUCT_CODE", currencySupportType: "ALL_SUPPORTED", productCodeList: [], minLeverage: null, maxLeverage: null, orderTypeList: [] }];
}

function buildRaceParams({ isParticipantsNum = 1, isTotalPricePoolAmount = 0, totalPricePoolAmount = null } = {}) {
  return {
    isParticipantsNum: Number(isParticipantsNum) ? 1 : 0,
    isTotalPricePoolAmount: Number(isTotalPricePoolAmount) ? 1 : 0,
    totalPricePoolAmount: totalPricePoolAmount ?? null,
    rankType: "TRADING",
    currentUseStageId: null,
    participants: null,
    tradeAmount: null,
  };
}

function buildFixBonusPoolParams({ taskId, prizeId }) {
  return [
    {
      stageId: 1,
...
function loadBaselinePayload() {
  const filePath = path.join(repoRoot, "skills/weex-admin-ops/references/payload-baselines/race-competition.activity-config.baseline.json");
...
async function raceOnline(api, config, activityId) {
...
async function raceOffline(api, config, activityId) {
...
async function raceDelete(api, config, activityId) {
...
async function unbindDependencies(api, activityId, { applyConfigId = 2442 } = {}) {
  ...
  if ("registerTemplateId" in patched) patched.registerTemplateId = Number(applyConfigId);
...
async function attemptCleanup({ config, created }) {
  ...
  try {
    if (created.activityId) {
      cleanup.unbind = await unbindDependencies(api, created.activityId).catch(error => ({ ok: false, error: error.message }));
      cleanup.deleteActivity = await raceDelete(api, config, created.activityId).catch(error => ({ ok: false, error: error.message }));
    }
    ...
    if (created.registerTemplateId) {
      cleanup.deleteRegisterTemplate = await deleteRegisterTemplate(api, created.registerTemplateId).catch(error => ({ ok: false, error: error.message }));
    }
    return cleanup;
  } finally {
...
async function run() {
  ...
  const plan = {
    ...
    dependencies: ["报名模板(从零)", "赠金奖品(从零)", "交易量任务(从零)", "图片上传(uploadImgReplace)"],
  };
  if (args.dryRun) {
  ...
  try {
    ...
    const task = await runChildJson([
      "skills/weex-admin-ops/scripts/create-race-trading-volume-task-from-scratch-fast-api.mjs",
      "--required-volume",
    ...
    steps.push({ name: "create_race_trading_volume_task_from_scratch", ok: task.ok, taskId: task.json?.created?.id || null });
    ...
    payload.requirements = buildRequirements();
    payload.rankType = "TRADING";
    payload.bonusPoolType = "FIXED";
    payload.raceParams = buildRaceParams({ isParticipantsNum: 1, isTotalPricePoolAmount: 0, totalPricePoolAmount: null });
    payload.raceRankingParams = { isShow: 1, minRank: 1, maxRank: 10 };
    payload.raceFixBonusPoolParams = buildFixBonusPoolParams({ taskId: created.taskId, prizeId: created.prizeId });

    const shareContent = "自动化分享文案";
    const agentShareContent = "自动化代理分享文案";
    const raceShareContent = "自动化竞速分享文案";
    ...
    payload.shareContent = shareContent;
    payload.agentShareContent = agentShareContent;
    payload.raceShareContent = raceShareContent;
    payload.intro = "<p>自动化活动规则</p>";

    if (Array.isArray(payload.activityConfigI18n)) {
      payload.activityConfigI18n = payload.activityConfigI18n.map(item => ({
        ...item,
        title: String(item?.lang || "") === "zh_CN" ? title : (item?.title || title),
        subTitle: String(item?.lang || "") === "zh_CN" ? subTitle : (item?.subTitle || subTitle),
        webBannerUrl: uploadedBanner,
        appBannerUrl: uploadedBanner,
        webShareUrl: uploadedBanner,
        appShareUrl: uploadedBanner,
        shareContent: item?.shareContent || shareContent,
        agentShareContent: item?.agentShareContent || agentShareContent,
        raceShareContent: item?.raceShareContent || raceShareContent,
      }));
    }

    let createRes = await createActivityConfigWithRetry(api, payload, { maxRetries: 6 });
    if (isAuthFailureResponse(createRes)) {
      steps.push({ name: "auth_refresh_before_create_retry", ok: true, reason: "create_activity_401" });
      await api.close().catch(() => {});
      api = await createAdminApiSession({ config, requireApiLogin: true });
      createRes = await createActivityConfigWithRetry(api, payload, { maxRetries: 6 });
    }
    const createOk = createRes.body?.code === 200;
    steps.push({ name: "create_race_activity", ok: createOk, response: { code: createRes.body?.code ?? null, msg: createRes.body?.msg || "" }, alias, title });
    if (!createOk) {
      throw new Error(`Create RACE_COMPETITION activity failed: ${JSON.stringify({ code: createRes.body?.code, msg: createRes.body?.msg || createRes.body?.message })}`);
    }

    const verifyList = await api.get(`/prod-api/activity/config/list?pageNum=1&pageSize=1&type=RACE_COMPETITION&showUrl=${encodeURIComponent(alias)}`);
    const row = firstRow(verifyList);
    const activityId = String(row?.activityId || row?.id || "");
    if (!activityId) throw new Error(`Created RACE_COMPETITION activity not found by alias: ${alias}`);
    ...
    const stage0 = Array.isArray(d?.raceFixBonusPoolParams) ? d.raceFixBonusPoolParams[0] : null;
    const verify = {
      ok: d?.type === "RACE_COMPETITION" && Number(d?.applyConfigId || 0) > 0 && Number(stage0?.taskId || 0) > 0 && Number(stage0?.prizeId || 0) > 0,
      checks: { type: d?.type ?? null, applyConfigId: d?.applyConfigId ?? null, stage0TaskId: stage0?.taskId ?? null, stage0PrizeId: stage0?.prizeId ?? null },
    };
    ...
    const result = {
      ...
      created,
      links: [{ label: "后台交易竞速赛列表", url: `${config.baseUrl}/activities/speedRace` }],
    ...
    const md = writeResultMarkdown({
      ...
      title: "交易竞速赛(RACE_COMPETITION) 通用回归（从零配置）",
      testCase: {
        number: `UR-ADMIN-${result.caseId}`,
        description: "验证交易竞速赛(RACE_COMPETITION)在 staging 环境从零创建依赖与活动，完成草稿检查、上线/下线，并按需清理创建物。",
        preconditions: [
        ...
        tags: ["admin", "universal-regression", "from-scratch", "RACE_COMPETITION"],
      },
    ...
    const md = writeResultMarkdown({
      ...
      title: "交易竞速赛(RACE_COMPETITION) 通用回归（失败）",
      testCase: {
        number: "UR-ADMIN-RACE_COMPETITION_universal_from_scratch",
        description: "验证交易竞速赛(RACE_COMPETITION)在 staging 环境从零创建依赖与活动，完成草稿检查、上线/下线，并按需清理创建物（失败场景输出）。",
        preconditions: [
          "环境：staging 活动后台可访问（默认 https://stg-activity.weex.tech）",
          "已配置并可登录：skills/weex-admin-ops/.env.local（WEEX_ADMIN_USERNAME/WEEX_ADMIN_PASSWORD/WEEX_ADMIN_GOOGLE_CODE）",
          "账号具备活动/任务/奖品/资源等配置权限",
