...
function usage() {
  return `Usage:
  node skills/weex-admin-ops/scripts/customized-activity-fast-api.mjs --action snapshot --activity-alias <alias>
  node skills/weex-admin-ops/scripts/customized-activity-fast-api.mjs --action inspect-template --activity-alias <alias>
  node skills/weex-admin-ops/scripts/customized-activity-fast-api.mjs --action create-draft --template-alias <alias> --title-prefix <text> --alias-prefix <text>
  node skills/weex-admin-ops/scripts/customized-activity-fast-api.mjs --action online --activity-alias <alias>
  node skills/weex-admin-ops/scripts/customized-activity-fast-api.mjs --action offline --activity-alias <alias>
  node skills/weex-admin-ops/scripts/customized-activity-fast-api.mjs --action delete --activity-alias <alias>
...
async function listByAlias(api, alias, type = "CUSTOMIZED") {
...
async function listById(api, id, type = "CUSTOMIZED") {
...
async function snapshot(api, args) {
  ...
  const detail = summarize(target.detail);
  return {
    ok: true,
    mode: "headless_api",
    finalUrl: "/activities/commission",
...
async function inspectTemplate(api, args) {
  ...
  const baseHints = {};
  for (const key of ["channelCategory", "guideTemplateId", "showUrl", "startTime", "endTime", "type", "showActivityCalendar"]) {
  ...
  return {
    ...
    moduleKeyHints: {
      base: { keys: sortedKeys(baseHints), sample: baseHints, applyConfigId: detail.applyConfigId ?? null },
...
async function createDraft(api, args) {
  ...
  const alias = String(args.aliasExact || `${args.aliasPrefix || "cz"}${Date.now().toString().slice(-8)}`).slice(0, 32);
  const title = String(args.titleExact || `${args.titlePrefix || "定制活动"}${ts.slice(-6)}`).slice(0, 120);
  ...
  if (created.body?.code !== 200) throw new Error(`Create CUSTOMIZED draft failed: ${JSON.stringify(created.body)}`);
  const row = await listByAlias(api, alias);
  const id = row?.activityId || row?.id;
  if (!id) throw new Error(`Created activity not found: ${alias}`);
  const verifyFirst = summarize(await detailById(api, id));
  return {
    ok: true,
    mode: "headless_api",
    finalUrl: "/activities/commission",
...
async function draftChecks(api, args) {
  ...
  return {
    ...
    searchChecks: {
      ...
      byType: { ok: true, type: "CUSTOMIZED", activityId: id },
...
async function online(api, args, config) {
  const target = await resolveTarget(api, args);
  const result = await api.post("/prod-api/activity/customized/online", { activityId: Number(target.id), totp: String(config.googleCode || "") });
  ...
  return {
    ...
    verifyItem: summarize(verifyItem),
  };
}

async function offline(api, args, config) {
  const target = await resolveTarget(api, args);
  const result = await api.post("/prod-api/activity/customized/offline", { activityId: Number(target.id), totp: String(config.googleCode || "") });
  const verifyItem = await detailById(api, target.id);
  return {
    ok: result.body?.code === 200 && String(verifyItem.status || "").toUpperCase() !== "ONLINE",
    mode: "headless_api",
...
async function remove(api, args, config) {
  const target = await resolveTarget(api, args);
  const result = await api.post("/prod-api/activity/customized/delete", { activityId: Number(target.id), totp: String(config.googleCode || "") });
