...
function usage() {
  return `Usage:
  # dry-run
  node skills/weex-admin-ops/scripts/create-race-trading-volume-task-fast-api.mjs --required-volume 1 --dry-run

  # create
  node skills/weex-admin-ops/scripts/create-race-trading-volume-task-fast-api.mjs --required-volume 1 --confirm-create

Options:
  --required-volume <n>   required; 交易量阈值（USDT）
  --template-id <id>      optional; default 3906 (RACE_COMPETITION TRADING_VOLUME)
  --name-prefix <text>    default 竞速赛_交易量
  --tag-prefix <text>     default rctr
...
function parseArgs() {
  ...
  args.templateId = args.templateId ? String(args.templateId) : "3906";
  args.namePrefix = args.namePrefix ? String(args.namePrefix) : "竞速赛_交易量";
  args.tagPrefix = args.tagPrefix ? String(args.tagPrefix) : "rctr";
...
function mutateTaskPayload(payload, args) {
  ...
  payload.label = tag;
  payload.remark = (args.remark || `自动化-竞速赛交易量任务(${args.requiredVolume}U)`).slice(0, 120);
