  ...
  try {
    ...
    const md = writeResultMarkdown({
      ...
      testCase: {
        ...
        description: "验证小丑牌(FLIP)在 staging 环境从零创建依赖与活动（含任务与虚拟奖品配置），完成草稿检查、上线/下线，并按需清理创建物。",
        preconditions: [
        ...
        tags: ["admin", "universal-regression", "from-scratch", "FLIP"],
      },
      summary: {
        ok: true,
    ...
    const md = writeResultMarkdown({
      ...
      testCase: {
        number: "UR-ADMIN-FLIP_universal_from_scratch",
        description: "验证小丑牌(FLIP)在 staging 环境从零创建依赖与活动（含任务与虚拟奖品配置），完成草稿检查、上线/下线，并按需清理创建物（失败场景输出）。",
        preconditions: [
          "环境：staging 活动后台可访问（默认 https://stg-activity.weex.tech）",
          "已配置并可登录：skills/weex-admin-ops/.env.local（WEEX_ADMIN_USERNAME/WEEX_ADMIN_PASSWORD/WEEX_ADMIN_GOOGLE_CODE）",
          "账号具备活动/任务/奖品/资源等配置权限",
