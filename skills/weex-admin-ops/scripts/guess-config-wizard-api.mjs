...
function usage() {
  ...
  node skills/weex-admin-ops/scripts/guess-config-wizard-api.mjs --wizard

  # Step 2: execute after confirmation (spec required)
  node skills/weex-admin-ops/scripts/guess-config-wizard-api.mjs --spec-file ./tmp/guess-spec.json --confirm
...
Options:
  ...
  --template-alias <alias> used by create_draft
  --template-id <id>       used by create_draft
  --title-prefix <text>    used by create_draft
  --alias-prefix <text>    used by create_draft
  --activity-alias <alias> used by online/offline/delete
  --activity-id <id>       used by online/offline/delete
  --start-offset-seconds <n> default 120; used by create_draft
  --end-days <n>           default 7; used by create_draft
...
function loadModuleNameMap() {
  const filePath = path.join(repoRoot, "skills/weex-admin-ops/references/mappings/guess-activity-modules.md");
...
function wizardOutput() {
  ...
  return {
    ...
    presets: [
      ...
      { preset: "online", risk: "高", description: "上线指定竞猜大赛活动（/activity/guess/online）" },
      { preset: "offline", risk: "高", description: "下线指定竞猜大赛活动（/activity/guess/offline）" },
      { preset: "delete", risk: "高", description: "删除指定竞猜大赛活动（/activity/guess/delete）" },
    ],
    confirmationRequired: ["活动标题/别名(showUrl)", "活动开始/结束时间", "依赖项写入（报名模板/奖品/活动任务）", "上线/下线/删除等写操作"],
    oneShotReplyTemplate,
    notes: [
      "full_create_* 会显式创建依赖项（报名模板/奖品/活动任务）并在 cleanup 时一并删除，避免污染 staging。",
      "模块级自由配置脚本：guess-activity-module-config-fast-api.mjs（snapshot -> spec -> update）。",
    ],
  };
}

function planPreset(args, skillRoot) {
  if (!args.confirm) throw new Error("需要用户确认：请加 --confirm 后才允许执行写操作。");
  const preset = String(args.preset || "");
  const node = process.execPath;
  const base = path.join(skillRoot, "scripts/guess-activity-fast-api.mjs");

  if (preset === "full_create_verify_delete") {
    const full = path.join(skillRoot, "scripts/create-guess-full-config-explicit-deps-fast-api.mjs");
    return {
      steps: [
        {
          script: full,
...
async function run() {
  ...
  const plan = planPreset(args, skillRoot);
  if (args.dryRun) {
