...
function usage() {
  return `Usage:
  node skills/weex-admin-ops/scripts/agent-activity-fast-api.mjs --action snapshot --activity-alias <alias>
  node skills/weex-admin-ops/scripts/agent-activity-fast-api.mjs --action inspect-template --activity-alias <alias>
  node skills/weex-admin-ops/scripts/agent-activity-fast-api.mjs --action create-draft --template-alias <alias> --title-prefix <text> --alias-prefix <text>
  node skills/weex-admin-ops/scripts/agent-activity-fast-api.mjs --action online --activity-alias <alias>
  node skills/weex-admin-ops/scripts/agent-activity-fast-api.mjs --action offline --activity-alias <alias>
  node skills/weex-admin-ops/scripts/agent-activity-fast-api.mjs --action delete --activity-alias <alias>
...
async function listByAlias(api, alias, type = "AGENT") {
...
async function listById(api, id, type = "AGENT") {
...
function summarizeActivityDetail(item) {
  ...
  return {
    ...
    activityLabelListCount: Array.isArray(d.activityLabelList) ? d.activityLabelList.length : null,
    ...
    showEndCountdown: d.showEndCountdown ?? null,
...
async function snapshot(api, args) {
  ...
  return {
    ...
    snapshot: {
      ...
      showEndCountdown: detail.showEndCountdown,
...
async function createDraft(api, args) {
  ...
  const alias = String(args.aliasExact || `${args.aliasPrefix || "ag"}${Date.now().toString().slice(-8)}`).slice(0, 32);
  const title = String(args.titleExact || `${args.titlePrefix || "人人代理活动"}${ts.slice(-6)}`).slice(0, 120);
  ...
  if (created.body?.code !== 200) throw new Error(`Create AGENT draft failed: ${JSON.stringify(created.body)}`);
...
async function draftChecks(api, args) {
  ...
  return {
    ...
    searchChecks: {
      ...
      byType: { ok: true, type: "AGENT", activityId: id },
...
async function remove(api, args, config) {
  const target = await resolveTarget(api, args);
  const result = await api.post("/prod-api/activity/agent/delete", { activityId: Number(target.id), totp: String(config.googleCode || "") });
