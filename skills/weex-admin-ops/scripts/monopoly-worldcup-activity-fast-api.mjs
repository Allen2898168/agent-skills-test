...
function usage() {
  return `Usage:
  node skills/weex-admin-ops/scripts/monopoly-worldcup-activity-fast-api.mjs --action snapshot --activity-alias <alias>
  node skills/weex-admin-ops/scripts/monopoly-worldcup-activity-fast-api.mjs --action inspect-template --activity-alias <alias>
  node skills/weex-admin-ops/scripts/monopoly-worldcup-activity-fast-api.mjs --action create-draft --template-alias <alias> --title-prefix <text> --alias-prefix <text>
  node skills/weex-admin-ops/scripts/monopoly-worldcup-activity-fast-api.mjs --action online --activity-alias <alias>
  node skills/weex-admin-ops/scripts/monopoly-worldcup-activity-fast-api.mjs --action offline --activity-alias <alias>
  node skills/weex-admin-ops/scripts/monopoly-worldcup-activity-fast-api.mjs --action delete --activity-alias <alias>
...
async function listByAlias(api, alias, type = "MONOPOLY_WORLD_CUP") {
...
async function listById(api, id, type = "MONOPOLY_WORLD_CUP") {
...
async function snapshot(api, args, config) {
  ...
  return {
    ...
    snapshot: {
      ...
      stage: detail.stage,
      monopolyListCount: detail.monopolyListCount,
    },
  };
}
...
async function inspectTemplate(api, args, config) {
  ...
  const first = monopolyList[0] || null;
  return {
    ok: true,
    mode: "headless_api",
    finalUrl: `${config.baseUrl}/activities/monopoly`,
    ...
    moduleKeyHints: {
      ...
      monopolyConfigs: { monopolyListCount: monopolyList.length, firstKeys: first ? sortedKeys(first) : [] },
...
async function createDraft(api, args, config) {
  ...
  const alias = String(args.aliasExact || `${args.aliasPrefix || "monopoly"}${Date.now().toString().slice(-8)}`).slice(0, 32);
  const title = String(args.titleExact || `${args.titlePrefix || "大富翁世界杯"}${ts.slice(-6)}`).slice(0, 120);
  ...
  if (created.body?.code !== 200) throw new Error(`Create MONOPOLY_WORLD_CUP draft failed: ${JSON.stringify({ code: created.body?.code, msg: created.body?.msg || created.body?.message })}`);
...
async function draftChecks(api, args, config) {
  ...
  const list = Array.isArray(item.monopolyList) ? item.monopolyList : [];
  const checklist = [
    { ok: item.type === "MONOPOLY_WORLD_CUP", key: "type", expected: "MONOPOLY_WORLD_CUP", actual: item.type ?? null },
    ...
    { ok: list.length >= 1, key: "monopolyList", expected: ">=1", actual: list.length },
  ];
  const failed = checklist.filter(it => !it.ok);
  return {
    ok: failed.length === 0,
    mode: "headless_api",
    finalUrl: `${config.baseUrl}/activities/monopoly`,
...
async function online(api, args, config) {
  const target = await resolveTarget(api, args);
  const res = await api.post("/prod-api/activity/monopoly/online", { activityId: Number(target.id), totp: String(config.googleCode || "") });
  if (res.body?.code !== 200) throw new Error(`Online MONOPOLY_WORLD_CUP failed: ${JSON.stringify({ code: res.body?.code, msg: res.body?.msg || res.body?.message })}`);
  return { ok: true, mode: "headless_api", finalUrl: `${config.baseUrl}/activities/monopoly`, activityId: target.id, alias: target.alias, response: { code: res.body.code, msg: res.body.msg || "" } };
}

async function offline(api, args, config) {
  const target = await resolveTarget(api, args);
  const res = await api.post("/prod-api/activity/monopoly/offline", { activityId: Number(target.id), totp: String(config.googleCode || "") });
  if (res.body?.code !== 200) throw new Error(`Offline MONOPOLY_WORLD_CUP failed: ${JSON.stringify({ code: res.body?.code, msg: res.body?.msg || res.body?.message })}`);
  return { ok: true, mode: "headless_api", finalUrl: `${config.baseUrl}/activities/monopoly`, activityId: target.id, alias: target.alias, response: { code: res.body.code, msg: res.body.msg || "" } };
}

async function del(api, args, config) {
  const target = await resolveTarget(api, args);
  const res = await api.post("/prod-api/activity/monopoly/delete", { activityId: Number(target.id), totp: String(config.googleCode || "") });
  if (res.body?.code !== 200) throw new Error(`Delete MONOPOLY_WORLD_CUP failed: ${JSON.stringify({ code: res.body?.code, msg: res.body?.msg || res.body?.message })}`);
  return { ok: true, mode: "headless_api", finalUrl: `${config.baseUrl}/activities/monopoly`, activityId: target.id, alias: target.alias, response: { code: res.body.code, msg: res.body.msg || "" } };
}
...
async function run() {
  ...
  try {
    ...
    if (args.action === "inspect-template") {
      printJson(await inspectTemplate(api, args, config));
      return 0;
    }
