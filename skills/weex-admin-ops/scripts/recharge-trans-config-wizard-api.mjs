...
function usage() {
  ...
  node skills/weex-admin-ops/scripts/recharge-trans-config-wizard-api.mjs --wizard

  # Step 2: execute after confirmation (spec required)
  node skills/weex-admin-ops/scripts/recharge-trans-config-wizard-api.mjs --spec-file ./tmp/recharge-trans-spec.json --confirm
...
function loadRechargeTransModuleNameMap() {
  const refPath = path.join(repoRoot, "skills/weex-admin-ops/references/mappings/recharge-trans-activity-modules.md");
...
function buildWizardMenu() {
  ...
  return {
    ...
    presets: [
      ...
      { preset: "online", risk: "高", description: "上线指定充值交易活动（/activity/rechargeTrans/online）" },
      { preset: "offline", risk: "高", description: "下线指定充值交易活动（/activity/rechargeTrans/offline）" },
      { preset: "delete", risk: "高", description: "删除指定充值交易活动（/activity/rechargeTrans/delete）" },
    ],
    ...
    notes: [
      ...
      "模块级自由配置脚本：recharge-trans-activity-module-config-fast-api.mjs（snapshot -> spec -> update）。",
    ],
  };
}
...
function planPreset(args) {
  ...
  const base = "skills/weex-admin-ops/scripts/recharge-trans-activity-fast-api.mjs";

  if (preset === "full_create_verify_delete") {
    const full = "skills/weex-admin-ops/scripts/create-recharge-trans-full-config-explicit-deps-fast-api.mjs";
    return {
      steps: [
        {
          script: full,
