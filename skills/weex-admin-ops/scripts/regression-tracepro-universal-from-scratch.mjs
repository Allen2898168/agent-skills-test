...
function usage() {
  return `Usage:
  # dry-run (no writes)
  node skills/weex-admin-ops/scripts/regression-tracepro-universal-from-scratch.mjs --dry-run

  # run (writes)
  node skills/weex-admin-ops/scripts/regression-tracepro-universal-from-scratch.mjs --confirm-run

Options:
  --title-prefix <text>       default 小活动通用回归
  --alias-prefix <text>       default tpr
...
function parseArgs() {
  ...
  args.titlePrefix = args.titlePrefix ? String(args.titlePrefix) : "小活动通用回归";
  args.aliasPrefix = args.aliasPrefix ? String(args.aliasPrefix) : "tpr";
...
function loadBaselinePayload() {
  const filePath = path.join(repoRoot, "skills/weex-admin-ops/references/payload-baselines/trace-pro.activity-config.baseline.json");
  ...
  try {
    ...
    if (Array.isArray(created.resourceCardIds) && created.resourceCardIds.length) {
      ...
      for (const id of created.resourceCardIds) {
        cleanup.deleteResourceCards.push(await deleteResourceCard(api, id).catch(error => ({ ok: false, error: error.message, id })));
      }
    }
    return cleanup;
  } finally {
...
async function run() {
  ...
  const plan = {
    ...
    dependencies: ["报名模板(从零)", "赠金奖品(从零)", "交易量任务(从零)", "资源卡(从零)", "图片上传(uploadImgReplace)"],
  };
  if (args.dryRun) {
  ...
  try {
    ...
    const task = await runChildJson([
      "skills/weex-admin-ops/scripts/create-tracepro-trading-volume-task-from-scratch-fast-api.mjs",
      "--required-volume",
    ...
    steps.push({ name: "create_tracepro_trading_volume_task_from_scratch", ok: task.ok, taskId: task.json?.created?.id || null });
    ...
    // replace top-level image + share fields for easier review
    ...
    steps.push({ name: "create_trace_pro_activity", ok: createOk, response: { code: createRes.body?.code ?? null, msg: createRes.body?.msg || "" }, alias, title });
    if (!createOk) throw new Error(`Create TRACE_PRO activity failed: ${JSON.stringify({ code: createRes.body?.code, msg: createRes.body?.msg || createRes.body?.message })}`);

    const verifyList = await api.get(`/prod-api/activity/config/list?pageNum=1&pageSize=1&type=TRACE_PRO&showUrl=${encodeURIComponent(alias)}`);
    const row = firstRow(verifyList);
    const activityId = String(row?.activityId || row?.id || "");
    if (!activityId) throw new Error(`Created TRACE_PRO activity not found by alias: ${alias}`);
    ...
    const verify = {
      ok: d?.type === "TRACE_PRO" && Number(d?.applyConfigId || 0) > 0 && firstMiniTaskId > 0 && (args.resourceCards <= 0 || resourceCount > 0),
      checks: { type: d?.type ?? null, applyConfigId: d?.applyConfigId ?? null, firstMiniTaskId: firstMiniTaskId || null, resourceCount },
    };
    ...
    const result = {
      ...
      created,
      links: [{ label: "后台小活动列表", url: `${config.baseUrl}/activities/tracePro` }],
    ...
    const md = writeResultMarkdown({
      ...
      title: "小活动(TRACE_PRO) 通用回归（从零配置）",
      testCase: {
        number: `UR-ADMIN-${result.caseId}`,
        description: "验证小活动(TRACE_PRO)在 staging 环境从零创建依赖与活动（含任务/资源卡与奖品），完成草稿检查、上线/下线，并按需清理创建物。",
        preconditions: [
        ...
        tags: ["admin", "universal-regression", "from-scratch", "TRACE_PRO"],
      },
    ...
    const md = writeResultMarkdown({
      ...
      title: "小活动(TRACE_PRO) 通用回归（失败）",
      testCase: {
        number: "UR-ADMIN-TRACE_PRO_universal_from_scratch",
        description: "验证小活动(TRACE_PRO)在 staging 环境从零创建依赖与活动（含任务/资源卡与奖品），完成草稿检查、上线/下线，并按需清理创建物（失败场景输出）。",
        preconditions: [
          "环境：staging 活动后台可访问（默认 https://stg-activity.weex.tech）",
          "已配置并可登录：skills/weex-admin-ops/.env.local（WEEX_ADMIN_USERNAME/WEEX_ADMIN_PASSWORD/WEEX_ADMIN_GOOGLE_CODE）",
          "账号具备活动/任务/奖品/资源等配置权限",
