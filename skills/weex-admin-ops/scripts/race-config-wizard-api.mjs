...
function usage() {
  ...
  node skills/weex-admin-ops/scripts/race-config-wizard-api.mjs --wizard

  # Step 2: execute after confirmation (spec required)
  node skills/weex-admin-ops/scripts/race-config-wizard-api.mjs --spec-file ./tmp/race-spec.json --confirm
...
function buildWizardMenu() {
  ...
  return {
    domain: "活动列表 / 交易竞速赛(RACE_COMPETITION) API 向导（进行中）",
    ...
    presets: [
      ...
      { preset: "online", risk: "高", description: "上线指定交易竞速赛活动（复用 /activity/competition/online）" },
      { preset: "offline", risk: "高", description: "下线指定交易竞速赛活动（复用 /activity/competition/offline）" },
      { preset: "delete", risk: "高", description: "删除指定交易竞速赛活动（复用 /activity/competition/delete）" },
    ],
    ...
    notes: [
      ...
      "模块级自由配置脚本：race-activity-module-config-fast-api.mjs（snapshot -> spec -> update）。",
    ],
  };
}
...
function planPreset(args) {
  ...
  if (preset === "full_create_verify_delete") {
    const full = "skills/weex-admin-ops/scripts/create-race-full-config-explicit-deps-fast-api.mjs";
    return {
      steps: [
        {
          script: full,
