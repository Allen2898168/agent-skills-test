...
function usage() {
  return `Usage:
  # dry-run
  node skills/weex-admin-ops/scripts/create-recharge-trans-trading-volume-task-fast-api.mjs --required-volume 1 --dry-run

  # create
  node skills/weex-admin-ops/scripts/create-recharge-trans-trading-volume-task-fast-api.mjs --required-volume 1 --confirm-create

Options:
  --required-volume <n>   required; 交易量阈值（USDT）
  --template-id <id>      optional; if omitted, auto-pick a RECHARGE_TRANS_TASK(12) TRADING_VOLUME template
  --name-prefix <text>    default 充值交易_交易量
  --tag-prefix <text>     default rttr
...
function parseArgs() {
  ...
  args.namePrefix = args.namePrefix ? String(args.namePrefix) : "充值交易_交易量";
  args.tagPrefix = args.tagPrefix ? String(args.tagPrefix) : "rttr";
  args.remark = args.remark ? String(args.remark) : "";
  return args;
}

async function taskList(api, { pageNum = 1, pageSize = 10, activityTypeCode = 12 } = {}) {
...
async function pickTradingVolumeTemplate(api, args) {
  ...
  const list = await taskList(api, { pageNum: 1, pageSize: 10, activityTypeCode: 12 });
  ...
  if (first?.id) {
    ...
    throw new Error(`未找到 RECHARGE_TRANS_TASK(12) 的 TRADING_VOLUME 任务模板；fallback 模板 requirement[0].type=${requirement0?.type || "<missing>"}，请手动指定 --template-id`);
  }
  throw new Error("未找到可用于 clone 的 RECHARGE_TRANS_TASK(12) 任务模板；请先在后管创建至少一条充值交易活动任务，或直接指定 --template-id");
}
...
function mutateTaskPayload(payload, args) {
  ...
  payload.label = tag;
  payload.remark = (args.remark || `自动化-充值交易交易量任务(${args.requiredVolume}U)`).slice(0, 120);
