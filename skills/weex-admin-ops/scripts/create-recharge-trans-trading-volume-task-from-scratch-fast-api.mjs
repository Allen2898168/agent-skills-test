...
function usage() {
  return `Usage:
  # dry-run
  node skills/weex-admin-ops/scripts/create-recharge-trans-trading-volume-task-from-scratch-fast-api.mjs --required-volume 1 --dry-run

  # create
  node skills/weex-admin-ops/scripts/create-recharge-trans-trading-volume-task-from-scratch-fast-api.mjs --required-volume 1 --confirm-create

Options:
  --required-volume <n>   required; 交易量阈值（USDT）
  --name-prefix <text>    default 充值交易_交易量
  --tag-prefix <text>     default rt_tv_s
...
function parseArgs() {
  ...
  args.tagPrefix = args.tagPrefix ? String(args.tagPrefix) : "rt_tv_s";
...
function buildRequirement(requiredVolume) {
  return {
    ...
    volumeCountType: ["FEE"],
    orderTypeList: ["MANUAL", "API", "BROKER"],
    leverVolumeMin: null,
    leverVolumeMax: null,
    isFirst: 0,
    isFirstStroke: null,
...
function buildTaskPayload(args) {
  ...
  return {
    ...
    remark: (args.remark || `自动化-充值交易交易量任务(${args.requiredVolume}U)`).slice(0, 120),
    conditions: {
      allowAgentList: [],
      allowUserList: [],
      allowAreaList: [],
...
async function run() {
  ...
  const plan = { mode: "headless_api", requiredVolume: args.requiredVolume, writes: { create: true }, preview: { name: payload.name, label: payload.label, activityType: payload.activityType, taskType: payload.taskType } };
  ...
  try {
    ...
    printJson({
      ...
      created: { id, name: payload.name, tag: payload.label, requiredVolume: args.requiredVolume, activityType: payload.activityType, taskType: payload.taskType },
