...
function usage() {
  ...
  node skills/weex-admin-ops/scripts/competition-config-wizard-api.mjs --wizard

  # Step 2: execute after confirmation (spec required)
  node skills/weex-admin-ops/scripts/competition-config-wizard-api.mjs --spec-file ./tmp/competition-spec.json --confirm
...
Options:
  ...
  --cleanup                delete the created activity after verify (default true for full_create_verify_delete)
...
function loadCompetitionModuleNameMap() {
  const refPath = path.join(repoRoot, "skills/weex-admin-ops/references/mappings/competition-activity-modules.md");
...
function buildWizardMenu() {
  ...
  return {
    ...
    presets: [
      ...
      { preset: "online", risk: "高", description: "上线指定交易大赛活动" },
      { preset: "offline", risk: "高", description: "下线指定交易大赛活动" },
      { preset: "delete", risk: "高", description: "删除指定交易大赛活动" },
    ],
    confirmationRequired: [
      "活动标题/别名(showUrl)",
      "活动开始/结束时间",
      "依赖项写入（报名模板/奖品/任务等：待补齐）",
      "上线/下线/删除等写操作",
    ],
    oneShotReplyTemplate,
    notes: [
      "该向导当前处于“逐活动类型沉淀”阶段：已提供活动级 create/online/offline/delete 的 API 脚本入口。",
      "full_create_* 会显式创建依赖项（报名模板/奖品/任务）并在 cleanup 时一并删除，避免污染 staging。",
      "字段级中文映射入口：references/mappings/competition-activity-fields.md；模块级自由配置脚本：competition-activity-module-config-fast-api.mjs。",
    ],
  };
}
...
function planPreset(args) {
  ...
  const base = "skills/weex-admin-ops/scripts/competition-activity-fast-api.mjs";

  if (preset === "full_create_verify_delete") {
    const full = "skills/weex-admin-ops/scripts/create-competition-full-config-explicit-deps-fast-api.mjs";
    return {
      steps: [
        {
          script: full,
  ...
  if (preset === "online" || preset === "offline" || preset === "delete") {
    ...
    return {
      steps: [
        { script: base, command: [node, base, "--action", preset, ...(args.activityAlias ? ["--activity-alias", args.activityAlias] : []), ...(args.activityId ? ["--activity-id", args.activityId] : [])] },
      ],
    };
  }
...
function runPlan(plan) {
  ...
  for (const step of plan.steps) {
    ...
    const json = (() => {
      try { return JSON.parse(stdout.split("\n").filter(Boolean).slice(-1)[0]); } catch { return null; }
    })();
    const createdAlias = json?.alias || json?.created?.alias || json?.activityAlias || "";
    const createdId = json?.activityId || json?.created?.activityId || "";
    if (plan.followUp?.action === "draft-checks") {
      const check = spawnSync(process.execPath, ["skills/weex-admin-ops/scripts/competition-activity-fast-api.mjs", "--action", "draft-checks", ...(createdAlias ? ["--activity-alias", createdAlias] : []), ...(createdId ? ["--activity-id", String(createdId)] : [])], { cwd: repoRoot, stdio: "pipe", encoding: "utf8" });
      if (check.status !== 0) return { ok: false, executed, error: "draft_checks_failed", stdout: check.stdout, stderr: check.stderr };
      if (plan.followUp.cleanup) {
        const del = spawnSync(process.execPath, ["skills/weex-admin-ops/scripts/competition-activity-fast-api.mjs", "--action", "delete", ...(createdAlias ? ["--activity-alias", createdAlias] : []), ...(createdId ? ["--activity-id", String(createdId)] : [])], { cwd: repoRoot, stdio: "pipe", encoding: "utf8" });
        if (del.status !== 0) return { ok: false, executed, error: "cleanup_delete_failed", stdout: del.stdout, stderr: del.stderr };
      }
      return { ok: true, executed, created: { activityId: createdId || null, alias: createdAlias || null }, cleanup: Boolean(plan.followUp.cleanup) };
    }
    return { ok: true, executed, created: { activityId: createdId || null, alias: createdAlias || null } };
  }
