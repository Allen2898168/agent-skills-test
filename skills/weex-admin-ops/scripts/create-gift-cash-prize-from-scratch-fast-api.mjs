...
function usage() {
  return `Usage:
  # dry-run
  node skills/weex-admin-ops/scripts/create-gift-cash-prize-from-scratch-fast-api.mjs --dry-run

  # create
  node skills/weex-admin-ops/scripts/create-gift-cash-prize-from-scratch-fast-api.mjs --confirm-create

Options:
  --name-prefix <text>   default 自动化-赠金
  --alias-prefix <text>  default api_gc
...
function parseArgs() {
  ...
  args.namePrefix = args.namePrefix ? String(args.namePrefix) : "自动化-赠金";
  args.aliasPrefix = args.aliasPrefix ? String(args.aliasPrefix) : "api_gc";
...
async function run() {
  ...
  const plan = {
    mode: "headless_api",
    prize: { type: "GIFT_CASH", subtype: "GIFT_CASH", efficientDay: args.efficientDay },
    writes: { create: true },
  };
  if (args.dryRun) {
  ...
  if (!args.confirmCreate) throw new Error("需要用户确认：请加 --confirm-create 后才允许创建赠金奖品。");
