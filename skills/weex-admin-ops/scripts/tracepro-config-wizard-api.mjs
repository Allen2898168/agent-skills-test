...
function usage() {
  ...
  node skills/weex-admin-ops/scripts/tracepro-config-wizard-api.mjs --wizard

  # Step 2: execute after confirmation (spec required)
  node skills/weex-admin-ops/scripts/tracepro-config-wizard-api.mjs --spec-file ./tmp/tracepro-spec.json --confirm
...
function loadTraceProModuleNameMap() {
  const refPath = path.join(repoRoot, "skills/weex-admin-ops/references/mappings/tracepro-activity-modules.md");
...
function buildWizardMenu() {
  ...
  return {
    ...
    presets: [
      ...
      { preset: "online", risk: "高", description: "上线指定小活动型活动（/activity/tracePro/online）" },
      { preset: "offline", risk: "高", description: "下线指定小活动型活动（/activity/tracePro/offline）" },
      { preset: "delete", risk: "高", description: "删除指定小活动型活动（/activity/tracePro/delete）" },
    ],
    ...
    notes: [
      "full_create_* 会显式创建依赖项（报名模板/奖品/任务/资源卡）并在 cleanup 时一并删除，避免污染 staging。",
      "模块级自由配置脚本：tracepro-activity-module-config-fast-api.mjs（snapshot -> spec -> update）。",
    ],
  };
}
...
function planPreset(args) {
  ...
  const base = "skills/weex-admin-ops/scripts/tracepro-activity-fast-api.mjs";

  if (preset === "full_create_verify_delete") {
    const full = "skills/weex-admin-ops/scripts/create-tracepro-full-config-explicit-deps-fast-api.mjs";
    return {
      steps: [
        {
          script: full,
