...
function usage() {
  return `Usage:
  # dry-run
  node skills/weex-admin-ops/scripts/create-guess-trading-volume-task-from-scratch-fast-api.mjs --required-volume 1 --dry-run

  # create
  node skills/weex-admin-ops/scripts/create-guess-trading-volume-task-from-scratch-fast-api.mjs --required-volume 1 --confirm-create

Options:
  --required-volume <n>   required; 交易量阈值（USDT）
  --name-prefix <text>    default 竞猜大赛_交易量
  --tag-prefix <text>     default guess_tv_s
...
function parseArgs() {
  ...
  args.namePrefix = args.namePrefix ? String(args.namePrefix) : "竞猜大赛_交易量";
  args.tagPrefix = args.tagPrefix ? String(args.tagPrefix) : "guess_tv_s";
...
function buildTaskPayload(args) {
  ...
  return {
    ...
    remark: (args.remark || `自动化-GUESS交易量任务(${args.requiredVolume}U)`).slice(0, 120),
    conditions: {
      allowAgentList: [],
      allowUserList: [],
      allowAreaList: [],
...
async function run() {
  ...
  const plan = { mode: "headless_api", activityType: "GUESS(22)", taskType: payload.taskType, requiredVolume: args.requiredVolume, writes: { create: true }, preview: { name: payload.name, label: payload.label } };
  if (args.dryRun) {
  ...
  try {
    ...
    printJson({ ok: true, mode: "headless_api", finalUrl: `${config.baseUrl}/activity/task`, created: { id: String(row.id), name: payload.name, tag: payload.label, requiredVolume: args.requiredVolume }, createdTaskDetail: detail, durationMs: Date.now() - startedAt });
    return 0;
  } finally {
