...
function usage() {
  return `Usage:
  node skills/weex-admin-ops/scripts/competition-activity-fast-api.mjs --action snapshot --activity-alias <alias>
  node skills/weex-admin-ops/scripts/competition-activity-fast-api.mjs --action inspect-template --activity-alias <alias>
  node skills/weex-admin-ops/scripts/competition-activity-fast-api.mjs --action create-draft --template-alias <alias> --title-prefix <text> --alias-prefix <text>
  node skills/weex-admin-ops/scripts/competition-activity-fast-api.mjs --action online --activity-alias <alias>
  node skills/weex-admin-ops/scripts/competition-activity-fast-api.mjs --action offline --activity-alias <alias>
  node skills/weex-admin-ops/scripts/competition-activity-fast-api.mjs --action delete --activity-alias <alias>
...
async function listByAlias(api, alias, type = "TRADING_COMPETITION") {
...
async function listById(api, id, type = "TRADING_COMPETITION") {
...
function summarizeActivityDetail(item) {
  ...
  return {
    ...
    showSchedule: item.showSchedule ?? null,
    activityScheduleCount: Array.isArray(item.activitySchedule) ? item.activitySchedule.length : 0,
    prizeCount: Array.isArray(item.prize) ? item.prize.length : 0,
...
async function snapshot(api, args) {
  ...
  return {
    ok: true,
    mode: "headless_api",
    finalUrl: "/activities/activity/competition/index",
    ...
    snapshot: {
      ...
      stage: detail.stage,
      scheduleCount: detail.activityScheduleCount,
      prizeCount: detail.prizeCount,
...
async function inspectTemplate(api, args) {
  ...
  return {
    ...
    moduleKeyHints: {
      ...
      schedule: summarizeArrayShape(detail.activitySchedule),
...
async function createDraft(api, args) {
  ...
  const alias = String(args.aliasExact || `${args.aliasPrefix || "tc"}${Date.now().toString().slice(-8)}`).slice(0, 32);
  const title = String(args.titleExact || `${args.titlePrefix || "交易大赛"}${ts.slice(-6)}`).slice(0, 60);
  ...
  if (created.body?.code !== 200) throw new Error(`Create trading competition draft failed: ${JSON.stringify(created.body)}`);
...
async function draftChecks(api, args) {
  ...
  return {
    ...
    searchChecks: {
      ...
      byType: { ok: true, type: "TRADING_COMPETITION", activityId: id },
