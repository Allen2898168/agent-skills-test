...
function usage() {
  return `Usage:
  # dry-run
  node skills/weex-admin-ops/scripts/create-customized-trading-volume-task-from-scratch-fast-api.mjs --required-volume 1 --award-prize-id 123 --dry-run

  # create
  node skills/weex-admin-ops/scripts/create-customized-trading-volume-task-from-scratch-fast-api.mjs --required-volume 1 --award-prize-id 123 --confirm-create

Options:
  --required-volume <n>    required; 合约交易量阈值（USDT）
  --award-prize-id <id>    required; 奖励绑定奖品ID（建议 POSITION_AIRDROP）
  --name-prefix <text>     default 定制活动_合约交易量
  --tag-prefix <text>      default cztr_s
...
function parseArgs() {
  ...
  args.namePrefix = args.namePrefix ? String(args.namePrefix) : "定制活动_合约交易量";
  args.tagPrefix = args.tagPrefix ? String(args.tagPrefix) : "cztr_s";
...
function buildTaskAward(awardPrizeId) {
  return {
    awardMode: "SINGLE",
...
function buildTaskPayload(args) {
  ...
  const content = `合约交易量≥${args.requiredVolume}U`.slice(0, 120);
  return {
    ...
    resetDay: null,
    resetTime: null,
    taskAward: buildTaskAward(args.awardPrizeId),
    ...
    remark: (args.remark || `自动化-定制活动合约交易量任务(${args.requiredVolume}U)`).slice(0, 120),
    conditions: {
      allowAgentList: [],
      allowUserList: [],
      allowAreaList: [],
...
async function run() {
  ...
  try {
    ...
    printJson({
      ...
      created: { id: String(row.id), name: payload.name, tag: payload.label, requiredVolume: args.requiredVolume, awardPrizeId: args.awardPrizeId },
