...
function usage() {
  ...
  node skills/weex-admin-ops/scripts/verify-newbie-api-full-config-staging.mjs --dry-run

  # Run min verify + cleanup (writes)
  node skills/weex-admin-ops/scripts/verify-newbie-api-full-config-staging.mjs --confirm-run --verify-level min

  # Run full verify + cleanup (writes; includes online/offline)
  node skills/weex-admin-ops/scripts/verify-newbie-api-full-config-staging.mjs --confirm-run --verify-level full --confirm-full-verify
...
function buildPlan(args) {
  ...
  return {
    verifyLevel,
    create: {
      script: "skills/weex-admin-ops/scripts/create-newbie-full-config-custom-tasks-fast-api.mjs",
      commandArgs: [
    ...
    verifyMin: [
      {
        label: "draft-checks",
        script: "skills/weex-admin-ops/scripts/newbie-activity-fast-api.mjs",
        commandArgs: ["skills/weex-admin-ops/scripts/newbie-activity-fast-api.mjs", "--action", "draft-checks", "--activity-id", "<created.activityId>"],
      },
    ],
    verifyFull:
      verifyLevel === "full"
        ? [
            {
              label: "online",
              script: "skills/weex-admin-ops/scripts/newbie-activity-fast-api.mjs",
              commandArgs: ["skills/weex-admin-ops/scripts/newbie-activity-fast-api.mjs", "--action", "online", "--activity-id", "<created.activityId>"],
            },
            {
              label: "offline",
              script: "skills/weex-admin-ops/scripts/newbie-activity-fast-api.mjs",
              commandArgs: ["skills/weex-admin-ops/scripts/newbie-activity-fast-api.mjs", "--action", "offline", "--activity-id", "<created.activityId>"],
            },
          ]
        : [],
    cleanup: {
      deleteActivity: {
        script: "skills/weex-admin-ops/scripts/newbie-activity-fast-api.mjs",
        commandArgs: ["skills/weex-admin-ops/scripts/newbie-activity-fast-api.mjs", "--action", "delete", "--activity-id", "<created.activityId>"],
      },
      deleteTasks: "<created.createdTaskIds via /prod-api/activity/task/:id>",
    },
  };
}
...
async function run() {
  ...
  if (args.dryRun) {
    printJson({
      ...
      plan,
      nextStep: "如需执行新手活动 create → min/full verify → cleanup，请加 --confirm-run。",
    });
    return 0;
  }
  ...
  const createdActivityId = String(created.json?.created?.activityId || created.json?.created?.activityId || created.json?.created?.id || "");
  const createdTaskIds = Array.isArray(created.json?.created?.createdTaskIds) ? created.json.created.createdTaskIds.map(String) : [];
  if (!createdActivityId) throw new Error("create step did not return created.activityId");

  // 2) min verify
  const draftChecks = await runChildJson(["skills/weex-admin-ops/scripts/newbie-activity-fast-api.mjs", "--action", "draft-checks", "--activity-id", createdActivityId]);
  if (!draftChecks.ok) {
    printJson({ ok: false, mode: "headless_api", phase: "min_verify", error: draftChecks.json?.error || "draft_checks_failed", created: created.json || null }, process.stderr);
    return 1;
  }
  const fullConfigChecks = draftChecks.json?.fullConfigChecks || null;
  ...
  if (String(plan.verifyLevel) === "full") {
    ...
    onlineRes = await runChildJson(["skills/weex-admin-ops/scripts/newbie-activity-fast-api.mjs", "--action", "online", "--activity-id", createdActivityId]);
    if (!onlineRes.ok) throw new Error(`online failed: ${onlineRes.json?.error || "unknown"}`);
    offlineRes = await runChildJson(["skills/weex-admin-ops/scripts/newbie-activity-fast-api.mjs", "--action", "offline", "--activity-id", createdActivityId]);
    if (!offlineRes.ok) throw new Error(`offline failed: ${offlineRes.json?.error || "unknown"}`);
  }

  // 4) cleanup
  const deleteActivity = await runChildJson(["skills/weex-admin-ops/scripts/newbie-activity-fast-api.mjs", "--action", "delete", "--activity-id", createdActivityId]);
  const api = await createAdminApiSession({ config, requireApiLogin: true });
  try {
    const deleteTasks = await Promise.all(createdTaskIds.map(id => deleteTask(api, id)));
    const ok = Boolean(created.ok) && Boolean(draftChecks.ok) && Boolean(deleteActivity.ok) && deleteTasks.every(item => item.ok) && (plan.verifyLevel !== "full" || (onlineRes?.ok && offlineRes?.ok));
    printJson(
      {
        ok,
        mode: "headless_api",
        ...
        minVerify: { draftChecks: draftChecks.json || null, fullConfigChecks },
