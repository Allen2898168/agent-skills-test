...
function usage() {
  ...
  node skills/weex-admin-ops/scripts/flip-config-wizard-api.mjs --wizard

  # Step 2: execute after confirmation (spec required)
  node skills/weex-admin-ops/scripts/flip-config-wizard-api.mjs --spec-file ./tmp/flip-spec.json --confirm
...
function loadModuleNameMap() {
  const filePath = path.join(repoRoot, "skills/weex-admin-ops/references/mappings/flip-activity-modules.md");
  ...
  return { source: "references/mappings/flip-activity-modules.md", map };
}

function loadFieldNameMap() {
  const filePath = path.join(repoRoot, "skills/weex-admin-ops/references/mappings/flip-activity-fields.md");
  if (!fs.existsSync(filePath)) return { source: "missing", map: {} };
  const md = fs.readFileSync(filePath, "utf8");
  const lines = md.split(/\r?\n/);
  ...
  return { source: "references/mappings/flip-activity-fields.md", map };
}
...
function wizardOutput() {
  ...
  return {
    domain: "活动列表 / 小丑牌活动(FLIP) API 向导（进行中）",
    ...
    presets: [
      ...
      { preset: "online", risk: "高", description: "上线指定小丑牌活动（/activity/flip/online）" },
      { preset: "offline", risk: "高", description: "下线指定小丑牌活动（/activity/flip/offline）" },
      { preset: "delete", risk: "高", description: "删除指定小丑牌活动（/activity/flip/delete）" },
    ],
    ...
    notes: [
      ...
      "模块级自由配置脚本：flip-activity-module-config-fast-api.mjs（snapshot -> spec -> update）。",
    ],
  };
}
...
function planPreset(args) {
  ...
  const base = "skills/weex-admin-ops/scripts/flip-activity-fast-api.mjs";

  if (preset === "full_create_verify_delete") {
    const full = "skills/weex-admin-ops/scripts/create-flip-full-config-explicit-deps-fast-api.mjs";
    return {
      steps: [
        {
          script: full,
