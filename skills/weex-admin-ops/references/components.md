...
3. Wait for a visible `.el-select-dropdown`; when multiple dropdown DOM nodes exist, use the last opened visible dropdown.
4. Click the exact visible option text.
5. Press `Escape` and/or click a blank dialog/page area when the dropdown may remain open.
6. Wait until the selected text appears in the control or the dependent field updates.
...
- Multiple visible dropdowns can briefly coexist after prior select operations; use the last opened visible dropdown, not the first match in the document.
- Leaving a select dropdown open can intercept later fields or submit buttons. Close it before continuing, especially for multi-selects.
...
4. Prefer `Escape` plus a blank dialog/page click when a plain click does not close the dropdown.
5. Continue only after the dropdown is closed and selected tags are visible.
...
- Reusing the same generic select helper is required: single-select and multi-select both select from the last opened visible dropdown, then close the dropdown before continuing when `blur` is requested.
...
Detailed component patterns for Joker activity multilingual and rich-text widgets are split out to keep this shared file small:
- `components/joker-activity.md`
