...
function inferFinanceAirdropGrantParams(query) {
  if (!/充值|发放|下发|增加|空投奖励|财务|FIN|合约|转进|转入|划转/i.test(query)) return {};
  ...
  const countMatch = query.match(/(?:创建|注册)?\s*(\d+)\s*个账号/);
  ...
  return {
    ...
    count: countMatch?.[1] || (/一\s*个账号|一个账号/.test(query) ? "1" : undefined),
