...
- 当用户用自然语言提出后台操作，例如“创建一个新手活动，用默认配置”，agent 必须：
  ...
  - 基于当前已沉淀链路，先整理本次操作的必填项、可配置项、可建议默认值和已知限制。
  - 判断还缺哪些必填信息。
  - 明确询问测试人员哪些配置使用默认值，哪些配置需要显式指定。
  - 先向测试人员确认或补全必要参数。
  - 如果用户给出的参数与已验证链路冲突、页面约束冲突或业务上明显不合理，必须先说明原因，不得直接执行。
  - 再执行实际页面操作。
...
- 当任何“第一次链路失败、重试后出现稳定路径”的情况被验证后，必须把这条稳定路径补充到对应 skill，并同步更新 `docs/session-handoff.md`。
